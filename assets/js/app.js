...
/* ===== Home sections: "popular" & "new" (safe add-on) ===== */
(function () {
  'use strict';

  // Prevent running twice if the file is loaded multiple times
  if (window.__sutrahHomeInit) return;
  window.__sutrahHomeInit = true;
  ...
  // Use your own render/card functions if they exist; otherwise fallback.
  function ensureRenderer() {
    if (typeof window.renderGrid === 'function') {
      // You already have a renderer somewhere in app.js — use it
      return window.renderGrid;
    }
    // Fallback mini renderer (keeps your DOM clean)
    return function renderGridFallback(containerId, list) {
      const el = document.getElementById(containerId);
      if (!el) return;
      el.innerHTML = '';
      list.forEach(p => {
        const card = (typeof window.buildProductCard === 'function')
          ? window.buildProductCard(p)   // use your existing card builder if present
          : productCardFallback(p);      // otherwise use the tiny fallback below
        el.appendChild(card);
      });
    };
  }

  function productCardFallback(p) {
    ...
    a.innerHTML = `
      ...
      <div class="card-body">
        ...
        <div class="price">${Number(p.price).toFixed(2)} د.أ</div>
        <button class="btn btn-primary">التفاصيل</button>
      </div>`;
    return a;
  }

  function fillHomeSections() {
    ...
    render('home-trending', popular);
    render('home-new', newest);
  }
  ...
  // Fill once the DOM is ready, and refill after hash navigation if needed
  ready(fillHomeSections);
