/* سترة — متجر عربي RTL (SPA)
   - الصفحة الرئيسية تعتمد على tags من products.js للأقسام (الأكثر طلباً / وصل حديثاً)
   - معرض صور في صفحة المنتج (مصغّرات + أسهم)
   - سلة + إتمام عبر واتساب أو CliQ
*/
...
(() => {
  ...
  // الرئيسية: تُبنى من tags
  function viewHome(){
    app.innerHTML = `
      <section class="hero floaters">
        <div>
          <span class="pill">جديدنا</span>
          <h1>سترة — أناقتكِ في سترك</h1>
          <p>عبايات عملية ومناسبة لكل طلّة: جامعة، دوام، مناسبة.</p>
          <div class="cta">
  ...
  function renderShopGrid(){
    ...
    let items = PRODUCTS.filter(p=>{
      const okCat = cat==="الكل" || (p.category||"").includes(cat);
  ...
  function viewProduct({ id }){
    ...
    app.innerHTML = `
      <section class="product section">
        <div class="gallery">
          <div class="main">
            <button class="nav prev" aria-label="السابق">‹</button>
            <img id="main-img" src="${p.images?.[0]||""}" alt="${p.title}">
            <button class="nav next" aria-label="التالي">›</button>
          </div>
          <div class="thumbs">
    ...
    function set(i){
      ...
      main.src = PRODUCTS.find(x=>x.id===id).images[cur];
  ...
  function viewCheckout(){
        ...
        <div class="checkout">
          <form id="co" class="check-card">
            ...
            <div class="pay-options form-row" aria-label="طريقة الدفع">
              <label><input type="radio" name="pay" value="wa" checked> واتساب — <span class="muted">الدفع عند الاستلام (تأكيد سريع)</span></label>
              <label><input type="radio" name="pay" value="cliq"> CliQ — <span class="muted">تحويل على نفس الرقم</span></label>
            </div>
            ...
            <p class="muted under-cta-note">
              سيُرسل الطلب مع رابط اتجاهات جوجل مابس للوصول مباشرة.
            </p>
          </form>
    ...
    function buildWhatsAppLink(form){
      ...
      lines.push(`رابط الاتجاهات: ${mapsDirectionsLink()}`);
      if(note) lines.push(`ملاحظة: ${note}`);
      lines.push("");
      lines.push("المنتجات:");
      state.cart.forEach(ci=>{
        const p = PRODUCTS.find(pp=> pp.id===ci.id);
        if(p) lines.push(`• ${p.title} — لون: ${ci.color || "-"}، مقاس: ${ci.size || "-"} × ${ci.qty}`);
      });
      ...
      lines.push("");
      lines.push(`الإجمالي الفرعي: ${money(sub)}`);
      lines.push(`الشحن: ${money(shp)}`);
      lines.push(`الإجمالي: ${money(sub + shp)}`);
      lines.push("");
      const paySel = $$("input[name='pay']").find(x=>x.checked)?.value;
      lines.push(paySel==="cliq" ? `طريقة الدفع: CliQ إلى ${PHONE_READ}` : "طريقة الدفع: عند الاستلام (تأكيد عبر واتساب)");
...
function viewAbout(){
  app.innerHTML = `
    <section class="section">
      <h2>من نحن — سترة</h2>
      <div class="card" style="padding:16px; line-height:1.9">
        <p><strong>سترةُ للعبايات ✨</strong></p>
        <p>
          صفحةٌ خُصِّصت لكل بنت وسيدة تعشق الأناقة والاحتشام. فكرتنا بسيطة:
          نخلي العباية خيارِكِ الأول وين ما كنتِ.
        </p>
        <p>
          سواء لطلعةٍ يومية، دوام، جامعة أو حتى مناسبة، مع
          <strong>سترة</strong>
          رح تلاقي العباية اللي تعكس شخصيتك وتمنحك حضورًا واثقًا وأناقةً متجددة.
        </p>
        <p>
          نحن مؤمنون إن العباية مش مجرد لبس؛ هي
          <strong>هوية وذوق وراحة</strong>.
          وهدفنا إنه <strong>سترة</strong> تكون خيارِكِ الدائم بأناقتك وحضورك. 🌹
        </p>
      </div>
    </section>
  `;
      ...
      <section class="section">
        ...
        <div class="card" style="padding:16px">
          <ul style="line-height:2">
            <li>المدن الرئيسية: رام الله، القدس، نابلس، الخليل…</li>
            <li>المدّة: <strong>3–6 أيام عمل</strong> حسب المنطقة.</li>
            <li>الرسوم: <strong>د.أ 3</strong> — مجاناً فوق <strong>د.أ 100</strong>.</li>
  ...
  function viewPolicy({ slug }){
    const content = {
      shipping: `
        ...
        <ul><li>٢–٤ أيام بالأردن، ٣–٦ أيام بفلسطين.</li><li>شحن مجاني فوق د.أ 100.</li><li>رابط اتجاهات في كل طلب.</li></ul>
      `,
      returns: `
        <h2>الإرجاع والاستبدال</h2>
        <p class="lead">استبدال مقاس خلال 7 أيام بحالته الأصلية. إرجاع خلال 14 يوماً.</p>
      `,
      privacy: `
        <h2>الخصوصية</h2>
        <p class="lead">نحترم خصوصيتك ونستخدم بياناتك فقط لإتمام الطلب ودعمك.</p>
      `
    }[slug] || `<h2>السياسة</h2><p>غير متوفّرة.</p>`;
...
// شريط علوي ذكي (يختفي عند النزول ويظهر عند الصعود)
(function(){
